...
var passwordCheckOld = function () 
{
   ...
   stringPassword = $("password").value; // Retrieve value from the DOM.
   ...
   stringOutput = ""; // Output box is starts empty.
 
   if (stringPassword.length < 8) { // Checks to see if the password is at least 8 characters long.
        stringOutput = "Password is too short. Password must contain at least 8 characters."; // Output error message indicating the password is too short.
   } else {
      if (arrayBadPasswords.includes(stringPassword)) { // Checks the user password against the array of bad passwords.
         stringOutput = "Sorry, this password is considered a bad password. Please enter a different password."; // Output error message indicating the password was in the array and is considered bad.
      } else {
         stringOutput = "Password is good."; // Output message indicating the user has entered a good and valid password.
      }
   }
   $("output").value = stringOutput; // Sends the output message to the DOM.
};
window.onload = function () 
{
   $("password").value = ""; // Password text box is empty upon loading the page.
   $("password").focus(); // Cursor defaults to the password text box upon loading the page.
   $("check_password").onclick = passwordCheckOld; // Executes the code to check user password upon clicking the button.
};